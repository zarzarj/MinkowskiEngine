...
def get_graph_feature_NoCoords(x, k=20, idx=None, use_coords=False):
    batch_size = x.size(0)
    num_points = x.size(2)
    # x = x.view(batch_size, -1, num_points)
    if idx is None:
...
class DGCNN_semseg_NoCoords(LightningModule):
    def __init__(self, in_channels=3, out_channels=20, **kwargs):
        super(DGCNN_semseg_NoCoords, self).__init__()
        self.save_hyperparameters()
        for name, value in kwargs.items():
            if name != "self":
                try:
    ...
    def forward(self, batch):
        ...
        batch_size = x.size(0)
        num_points = x.size(2)
        x = get_graph_feature_NoCoords(x, k=self.k, use_coords=True)   # (batch_size, 9, num_points) -> (batch_size, 9*2, num_points, k)
        x = self.conv1(x)                       # (batch_size, 9*2, num_points, k) -> (batch_size, 64, num_points, k)
        x = self.conv2(x)                       # (batch_size, 64, num_points, k) -> (batch_size, 64, num_points, k)
        x1 = x.max(dim=-1, keepdim=False)[0]    # (batch_size, 64, num_points, k) -> (batch_size, 64, num_points)
