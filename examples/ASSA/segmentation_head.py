...
class SceneSegHeadResNet(nn.Module):
    ...
    def forward(self, end_points):
        features = self.up0(end_points['l3_xyz'], end_points['l4_xyz'],
                            end_points['l3_mask'], end_points['l4_mask'], end_points['l4_features'])
        features = torch.cat([features, end_points['l3_features']], 1)
        features = self.up_conv0(features)

        features = self.up1(end_points['l2_xyz'], end_points['l3_xyz'],
                            end_points['l2_mask'], end_points['l3_mask'], features)
        features = torch.cat([features, end_points['l2_features']], 1)
        features = self.up_conv1(features)

        features = self.up2(end_points['l1_xyz'], end_points['l2_xyz'],
                            end_points['l1_mask'], end_points['l2_mask'], features)
        features = torch.cat([features, end_points['l1_features']], 1)
        features = self.up_conv2(features)

        features = self.up3(end_points['l0_xyz'], end_points['l1_xyz'],
                            end_points['l0_mask'], end_points['l1_mask'], features)
        features = torch.cat([features, end_points['l0_features']], 1)
...
class SceneSegHeadPointNet(nn.Module):
    def __init__(self, num_classes, in_channels):
        ...
        self.head = nn.Sequential(nn.Conv1d(in_channels, 32, kernel_size=1, bias=False),
                                  nn.BatchNorm1d(32),
                                  nn.ReLU(inplace=True),
